...
function LoginAPI(props) {
    ...
    useEffect(() => {
        if (props.APIDetailsLogin.user.length > 0) {
            ...
            const url = 'https://www.upkasia.com/?rest_route=/simple-jwt-login/v1/auth'
            fetch(url, {
                method: 'POST',
                body: formData
            })
                ...
                .then((data) => {
                    if (data['success'] == true) {
                        ...
                        setUrlToLogin(`https://www.upkasia.com/?rest_route=/simple-jwt-login/v1/autologin&jwt=${data['data']['jwt']}`)
                    }
                    else {
                        console.log(data)
            ...
            fetch(urlToLogin, {
                ...
                .then((response) => {
                    if (response.status == '200') {
                        ...
                        window.location.replace('https://ceedyyy.github.io/production-portfolio/')
                    }
                    else {
                        console.log('error')
                    }
